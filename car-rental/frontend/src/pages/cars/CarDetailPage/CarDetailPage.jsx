...
import { getCarById, getRatingsByCarId, createBooking, post, searchCars } from "@/services/api.js"
import {
  FaCarSide,
  FaUser,
  FaTachometerAlt,
...
const CarDetailPage = () => {
  ...
  useEffect(() => {
    const fetchCarDetail = async () => {
      try {
        ...
        // LOG DEBUG: Kiểm tra dữ liệu response từ API
        console.log('=== [FETCH] CAR DETAIL DEBUG ===');
        if (car) {
          console.table(car);
          if (car.supplier) {
            console.log('[FETCH] car.supplier:', car.supplier);
            if (car.supplier.userDetail) {
              console.log('[FETCH] car.supplier.userDetail:', car.supplier.userDetail);
            } else {
              console.warn('[FETCH] ⚠️ car.supplier.userDetail is NULL or UNDEFINED');
            }
          } else {
            console.warn('[FETCH] ⚠️ car.supplier is NULL or UNDEFINED');
          }
        } else {
          console.error('[FETCH] ❌ No car data in response');
        }
        setCar(car);
      } catch (error) {
        console.error('[FETCH] ❌ Error fetching car details:', error);
  ...
  // LOG DEBUG: Kiểm tra dữ liệu khi render
  useEffect(() => {
    console.log('=== [RENDER] CAR STATE DEBUG ===');
    if (car) {
      console.table(car);
      if (car.supplier) {
        console.log('[RENDER] car.supplier:', car.supplier);
        if (car.supplier.userDetail) {
          console.log('[RENDER] car.supplier.userDetail:', car.supplier.userDetail);
        } else {
          console.warn('[RENDER] ⚠️ car.supplier.userDetail is NULL or UNDEFINED');
        }
      } else {
        console.warn('[RENDER] ⚠️ car.supplier is NULL or UNDEFINED');
      }
    } else {
      console.warn('[RENDER] ⚠️ car is NULL');
    }
    console.log('==============================');
  }, [car]);
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-6 text-gray-600">
                  <div className="flex items-center bg-white rounded-xl px-4 py-2 shadow-md">
                    ...
                    <span className="ml-2 font-bold text-gray-800">{car.averageRating || 4.8}</span>
                    <span className="ml-1 text-sm">/5.0</span>
                  </div>
                  <div className="flex items-center bg-white rounded-xl px-4 py-2 shadow-md">
                    <FaComments className="mr-2 text-blue-500" />
                    <span className="font-medium">{ratings.length} đánh giá</span>
                  </div>
                  <div className="flex items-center bg-white rounded-xl px-4 py-2 shadow-md">
              ...
              <div className="flex gap-3 mt-6 overflow-x-auto pb-3 px-1">
                {car.images?.map((img, idx) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-20 h-16 rounded-xl overflow-hidden border-3 transition-all duration-300 ${
            ...
            {car.supplier ? (
              <div className="bg-white/95 backdrop-blur-xl rounded-2xl p-6 shadow-xl mb-8 border border-gray-100">
                ...
                <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
                  <div className="w-24 h-24 bg-gradient-to-br from-blue-400 to-purple-400 rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center text-white text-2xl font-bold">
                    {car.supplier.userDetail?.fullName?.charAt(0)?.toUpperCase() || car.supplier.username?.charAt(0)?.toUpperCase() || car.supplier.email?.charAt(0)?.toUpperCase() || "U"}
                  </div>
                  <div className="flex-1 text-center md:text-left">
                    <h3 className="text-xl font-bold text-gray-800 mb-1">{car.supplier.userDetail?.fullName || car.supplier.username || "Chủ xe"}</h3>
                    <div className="flex items-center justify-center md:justify-start gap-2 mb-3">
                      <span className="text-gray-600">@{car.supplier.username}</span>
                      {car.supplier.statusName && (
                        <span className={`ml-2 px-2 py-1 rounded-full text-xs font-medium ${car.supplier.statusName === 'Đang hoạt động' || car.supplier.statusName === 'Active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{car.supplier.statusName}</span>
                      )}
                    </div>
                    <div className="space-y-3">
                      {car.supplier.phone && (
                        <div className="flex items-center justify-center md:justify-start gap-3 text-gray-700">
                          ...
                          <span>{car.supplier.phone}</span>
                        </div>
                      )}
                      {car.supplier.email && (
                        <div className="flex items-center justify-center md:justify-start gap-3 text-gray-700">
                          <FaEnvelope className="text-blue-600" />
                          <span>{car.supplier.email}</span>
                        </div>
                      )}
                      {car.supplier.userDetail?.address && (
                        <div className="flex items-center justify-center md:justify-start gap-3 text-gray-700">
                          <FaMapMarkerAlt className="text-blue-600" />
                          <span>{car.supplier.userDetail.address}</span>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="bg-white/95 backdrop-blur-xl rounded-2xl p-6 shadow-xl mb-8 border border-gray-100 text-center text-gray-500">
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center text-sm text-gray-600 mb-3">
                    <StarRating rating={similarCar.averageRating || 4.8} />
                    <span className="ml-2">{similarCar.averageRating || 4.8}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <div>                                                                                  <div className="text-xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
