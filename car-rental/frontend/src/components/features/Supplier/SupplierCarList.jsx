...
const SupplierCarList = () => {
  ...
  // --- Thêm state cho popup bảo hiểm ---
  const [showInsurancePopup, setShowInsurancePopup] = useState(false);
  const [approvedCarData, setApprovedCarData] = useState(null);
  ...
  // Kiểm tra xe được duyệt và hiển thị popup bảo hiểm
  useEffect(() => {
    const checkApprovedCarsForInsurance = () => {
      const pendingCarInfo = localStorage.getItem('pendingCarForInsurance');
      if (pendingCarInfo) {
        const carInfo = JSON.parse(pendingCarInfo);
        
        // Tìm xe trong danh sách hiện tại
        const approvedCar = cars.find(car => {
          const carId = car.carId || car.id;
          const pendingCarId = carInfo.carId;
          return carId == pendingCarId && 
                 (car.status?.statusName?.toLowerCase() === 'available' || 
                  car.status?.statusName?.toLowerCase() === 'có sẵn' ||
                  car.statusName?.toLowerCase() === 'available' ||
                  car.statusName?.toLowerCase() === 'có sẵn');
        });

        if (approvedCar && carInfo.addInsuranceAfterSubmit) {
          // Xe đã được duyệt và user đã chọn thêm bảo hiểm
          setApprovedCarData({
            model: approvedCar.model || approvedCar.name,
            licensePlate: approvedCar.licensePlate,
            brand: approvedCar.brand?.brandName || approvedCar.brand,
            carId: approvedCar.carId || approvedCar.id
          });
          setShowInsurancePopup(true);
          
          // Xóa thông tin khỏi localStorage
          localStorage.removeItem('pendingCarForInsurance');
        }
      }
    };

    // Chỉ kiểm tra khi cars đã được load
    if (cars.length > 0) {
      checkApprovedCarsForInsurance();
    }
  }, [cars]);
  ...
  const handleInsuranceSubmit = async (insuranceData) => {
    try {
      ...
      toast.success('Đã thêm thông tin bảo hiểm thành công');
    } catch (error) {
      console.error('Error adding insurance:', error);
      toast.error('Có lỗi xảy ra khi thêm bảo hiểm');
      throw error;
    }
  };

  // Xử lý khi đóng popup bảo hiểm
  const handleInsuranceClose = () => {
    setShowInsurancePopup(false);
    setApprovedCarData(null);
  };
