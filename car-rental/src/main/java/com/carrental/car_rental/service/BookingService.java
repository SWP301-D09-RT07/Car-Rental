...
public class BookingService {
    ...
    public BookingDTO findById(Integer id) {
        ...
        Booking booking = bookingRepository.findByIdWithAllDetails(id)
    ...
    public List<BookingDTO> findAll() {
        ...
        return bookingRepository.findAll().stream()
                .map(b -> bookingRepository.findByIdWithAllDetails(b.getId())
                        .filter(bk -> !bk.getIsDeleted())
    ...
    public List<BookingDTO> findByCarId(Integer carId) {
        ...
        return bookingRepository.findByCarIdAndIsDeleted(carId, false).stream()
                .map(b -> bookingRepository.findByIdWithAllDetails(b.getId())
                        .filter(bk -> !bk.getIsDeleted())
                        .map(bookingMapper::toDTO)
                        .orElse(null))
    ...
    public List<BookingDTO> findRecentBookings(int size) {
        return bookingRepository.findAllByIsDeletedFalseOrderByBookingDateDesc(PageRequest.of(0, size)).stream()
                .map(b -> bookingRepository.findByIdWithAllDetails(b.getId())
                        .filter(bk -> !bk.getIsDeleted())
                        .map(bookingMapper::toDTO)
                        .orElse(null))
    ...
    public List<UserDTO> findRecentBookingUsers(int size) {
        logger.info("Fetching {} recent booking users", size);
