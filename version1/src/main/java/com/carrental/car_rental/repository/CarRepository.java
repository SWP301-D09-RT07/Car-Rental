...
public interface CarRepository extends JpaRepository<Car, Integer>, JpaSpecificationExecutor<Car> {
    @Query("SELECT c FROM Rating r " +
        "JOIN r.car c " +
        "WHERE c.isDeleted = false AND c.status.statusName = 'available' AND r.isDeleted = false " +
        "GROUP BY c " +
    ...
    @Query("SELECT c FROM Booking b " +
        "JOIN b.car c " +
        "WHERE c.isDeleted = false AND c.status.statusName = 'available' " +
        "GROUP BY c " +
        "ORDER BY COUNT(b.id) DESC")
    Page<Car> findPopularCars(Pageable pageable);

    @Query("SELECT c FROM Car c JOIN FETCH c.brand JOIN FETCH c.fuelType JOIN FETCH c.region JOIN FETCH c.status WHERE c.isDeleted = false AND c.status.statusName = 'available'")
    ...
    @Query("SELECT c FROM Car c JOIN FETCH c.brand JOIN FETCH c.fuelType JOIN FETCH c.region JOIN FETCH c.status WHERE c.brand.id = :brandId AND c.isDeleted = false AND c.status.statusName = 'available'")
    List<Car> findByBrand_IdAndIsDeletedFalse(Integer brandId);

    @Query("SELECT c FROM Car c JOIN FETCH c.brand JOIN FETCH c.fuelType JOIN FETCH c.region JOIN FETCH c.status WHERE c.fuelType.id = :fuelTypeId AND c.isDeleted = false AND c.status.statusName = 'available'")
    ...
    @Query("SELECT c FROM Car c JOIN FETCH c.supplier JOIN FETCH c.brand JOIN FETCH c.fuelType JOIN FETCH c.region JOIN FETCH c.status WHERE c.isDeleted = false AND c.status.statusName = 'available'")
    Page<Car> findAllWithRelations(Pageable pageable);

    @Query("SELECT c FROM Car c JOIN FETCH c.brand JOIN FETCH c.fuelType JOIN FETCH c.region JOIN FETCH c.status WHERE c.region.id = :regionId AND c.isDeleted = false AND c.status.statusName = 'available'")
    ...
    @Query("SELECT c FROM Car c " +
            ...
            "AND c.id != :carId " +
            "AND c.status.statusName = 'available' " +
    ...
    @Query("SELECT c FROM Car c " +
            ...
            "WHERE c.brand.id = :brandId " +
            "AND c.id != :carId " +
            "AND c.isDeleted = false AND c.status.statusName = 'available'")
